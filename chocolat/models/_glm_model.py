...
class ModelGlmmBinary(PyroModule):
    """
...
class GuideGlmmBinary(PyroModule):
    """
...
def prepare_data4binary_glmm(adata_dict, samples_dict, observations_dict,
    ...
    return prior_parametrisation_tensor_dict, region_indices_stacked, sample_indices_by_region, observations_tensor, observations_by_region

def train_glm_models(prior_parametrisation_tensor_dict,
                     region_indices_stacked,
                     sample_indices_by_region,
                     observations_tensor,
    ...
    model = ModelGlmmBinary(prior_parametrisation_tensor_dict['spot_sensitivity'],
            prior_parametrisation_tensor_dict['region_params'],
            region_indices_stacked,
            sample_indices_by_region,
           [observations_tensor.shape[1], 8])

    guide = GuideGlmmBinary(prior_parametrisation_tensor_dict['spot_sensitivity'],
                prior_parametrisation_tensor_dict['region_params'],
                region_indices_stacked,
                sample_indices_by_region,
               [observations_tensor.shape[1], 8])
    ...
    # move to device
    observations_tensor = observations_tensor.to(model.device)
    ...
    with tqdm(total=num_iterations, desc=f"Processing") as pbar:
        for i in range(num_iters):
            loss = svi.step(observations_by_region.T)
...
def _reindex_region_ids(region_ids, keys):
    """
    Reindex region IDs to make them unique across multiple samples.
    Keeps -1 unchanged to signify 'NA' or 'unassigned'.
    
    Parameters:
    - region_ids (dict): Dictionary containing region IDs for each sample as pandas Series.
    
    Returns:
    - new_region_ids (dict): Dictionary containing reindexed region IDs for each sample.
    - back_mapping (dict): Back mapping dictionary for reverse lookup of original IDs.
    - sample_mapping (dict): Mapping from new region IDs back to the sample they came from.
    """
    new_region_ids = {}
    back_mapping = {}
    sample_mapping = {}
...
def _spread_ihc_annotations(adata, key_name='IHC-GFP'):
    adata_obs = adata.obs.copy()
    # Step 1: Filter out rows where either 'histopath_annotation' or 'IHC-GFP' is NaN
    filtered_data = adata_obs.dropna(subset=['histo_annotation', key_name])

    # Step 2: Group by 'histopath_annotation' and 'IHC-GFP', and count occurrences
    grouped_counts = filtered_data.groupby(['histo_annotation', key_name]).size().reset_index(name='counts')

    # Step 3: Calculate proportion of each 'IHC-GFP' within each 'histopath_annotation'
    ...
    # Step 4: Identify the most abundant 'IHC-GFP' for each 'histopath_annotation' that exceeds the 15% threshold
    ...
    # Create a dictionary mapping 'histopath_annotation' to 'IHC-GFP'
    annotation_to_gfp = {row['histo_annotation']: row[key_name] for _, row in most_abundant_annotation.iterrows()}

    # Step 5: Map these 'IHC-GFP' labels back to the original DataFrame
